...
| 3 | **Git:** commit WIP on `tweak/today-focus-queue-layout` (`WIP: Session N blocked — {reason}`) or `git stash`. Do **not** merge to `main`. See [GIT_WORKFLOW.md](../00-constitution/governance/GIT_WORKFLOW.md). |
...
| Surfaces | `src/app/globals.css` + [DESIGN_SYSTEM.md](../05-design/DESIGN_SYSTEM.md) | Canonical tokens — map spec levels to these |
...
5. Adapt surface tokens to [DESIGN_SYSTEM.md](../05-design/DESIGN_SYSTEM.md) — do not ship a parallel `:root` block that fights globals.
