...
| 3 | Record the decision in `docs/current-phase/logs/decision-log.md` (one entry: docs live at `flowos/docs/`). |
...
|   | - **Option A (preferred):** `git branch 11-archive/flowos-old` from inside `FlowOS-old` if it is a git repo, push branch, then delete folder. |
|   | - **Option B:** Zip to `flowos/docs/11-archive/FlowOS-old.zip` (or external backup), then `Remove-Item -Recurse -Force ..\FlowOS-old`. |
| 4 | Add one line to `docs/current-phase/logs/decision-log.md` describing archive method and date. |
