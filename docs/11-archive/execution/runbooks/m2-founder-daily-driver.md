...
| 2 | **Record blocker** in `docs/current-phase/logs/decision-log.md`: date, session #, WP id, symptom, what was tried, next action. |
| 3 | **Git state:** commit WIP on the **session branch** (`WIP: Session N blocked — {reason}`) or `git stash`. Do **not** merge to `main`. See [GIT_WORKFLOW.md](../00-constitution/governance/GIT_WORKFLOW.md). |
...
- [ ] **15-name list** — knowledge workers who already use 2+ productivity apps; **not** developer friends (see [recruiting-pipeline.md](../ops/recruiting-pipeline.md) anti-personas).
...
Create `docs/current-phase/logs/friction-log.md` (or private sheet — if private, note location in decision log):
...
Use only when truly blocked. Record choice in `docs/current-phase/logs/decision-log.md`.
