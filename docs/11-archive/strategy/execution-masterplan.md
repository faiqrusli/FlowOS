...
**Status:** **Transitional historical reference — superseded as the implementation source by [MVP Implementation Masterplan](../07-strategy-and-delivery/mvp-implementation-masterplan.md).** It retains dated milestone context and unresolved-work inventory; no new implementation may start from it without explicit re-admission through the new masterplan. (Release gates and metric definitions: [../00-constitution/governance/GATES.md](../00-constitution/governance/GATES.md).)
**Role:** Historical detailed milestone delivery plan. The [FlowOS Roadmap](../07-strategy-and-delivery/roadmap.md) owns current outcome sequencing, commitment classes, and evidence gates; the [MVP Implementation Masterplan](../07-strategy-and-delivery/mvp-implementation-masterplan.md) owns the current pre-dogfood implementation sequence.
**Foundation:** Builds directly on `11-archive/foundation/FOUNDATION_VALIDATION_REPORT.md`. Its conclusions are treated as fact and are not re-argued here.
...
2. `docs/` is tracked in version control and pushed (either the `flowos/` repo relocated to workspace root, or docs moved into the repo — a 30-minute decision, decided once, recorded in `docs/current-phase/logs/decision-log.md`).
...
**Success metrics / exit criteria:**
- 5 users onboarded per [recruiting-pipeline.md](../execution/ops/recruiting-pipeline.md) persona criteria (knowledge workers who already use 2+ productivity apps; **not** developer friends).
- Every user: day-0 onboarding call (watch first 10 minutes), day-3 async check-in, day-7 interview (five standard questions in recruiting-pipeline), day-14 retrospective.
- **The gate: ≥ 3 of 5 return on day 7 unprompted** (per `GATES.md`, which remains authoritative).
...
**Explicitly NOT allowed:** New features users didn't ask for. Public launch activity of any kind. Recruiting beyond 5. Reacting to any single user's feature request (3+ users citing it as a churn risk is the bar, per `PRINCIPLES.md`).
...
**Exit criteria:** D7 > 30% sustained 4 weeks at 15 users; D14 > 20%; external-tool fallback decreasing week over week. Or: a second gate failure, triggering the simplification path (cut to Today + Tasks + Reflection, re-test with 3 fresh users — per `GATES.md` pivot rule).
...
| **GATES.md, SUCCESS_METRICS** | **Consolidated into governance/GATES.md** | Gates remain authoritative; see GATES.md |
...
| WP | Purpose | Effort |
|----|---------|--------|
| **WP-5.1 Retention dossier + decision memo** | All cohort data, interview synthesis, churn reasons; binary GO/CONTRACT decision, dated, in `docs/current-phase/logs/decision-log.md` | 1–2 d |
