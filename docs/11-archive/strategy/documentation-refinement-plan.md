...
|---|---|---|
| Constitution and product meaning | `00-constitution/Vision.md`, `00-constitution/`, `01-product/`, `02-systems/` | Leave Vision immutable; resolve any conflicts in the owning active document. |
...
| Design standards and implementation | `05-design/` plus `05-design/DESIGN_SYSTEM_V3*.md` and CSS | Use the Design Implementation Map to name one source per layer. |
| Engineering architecture | `06-engineering/` plus `06-engineering/TECHNICAL_ARCHITECTURE.md` and governance | Promote durable boundaries; keep stack and code notes implementation-specific. |
...
| Decisions, evidence, and reviews | `08-decisions/`, `09-evidence/`, `09-reviews/` | Preserve why, what was observed, and what changed as separate records. |
...
| Historical material | `11-archive/`, old strategy/design folders | Retain for context, label clearly, and never start new work from it directly. |
...
|---|---|
| `11-archive/strategy/execution-masterplan.md` | Archived historical context; new implementation begins only from the MVP Masterplan. |
| `04-features/FEATURE_INVENTORY.md` | Keep as detailed current snapshot until the Feature Catalog absorbs all still-valid facts. |
| `05-design/DESIGN_SYSTEM_V3*.md` | Keep as implementation references until stable rules are promoted; do not duplicate them wholesale. |
| `11-archive/review/design/` and `11-archive/design/` feature specs | Historical evidence only; create or migrate a canonical spec under `05-design/` when materially reused or changed. |
| `11-archive/project/03-future-enhancements.md` | Keep historical; re-admit only through the current feature catalog and a decision record. |
| `00-constitution/governance/` | Keep repository and operational guidance; do not treat it as replacement for 06-engineering standards. |
