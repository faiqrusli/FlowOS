...
Verdict per dimension, with drift findings. Credit where due: the `11-archive/foundation/GOVERNANCE_PASS_REPORT.md` drift-fixing pass was real — several older claims (module map, agenda card, build status) were already corrected. But this board found additional drift the internal audits missed.
...
`11-archive/foundation/CEO_REVIEW_JULY_2026.md` states core tables are "provably open" (`using (true)`). What the SQL files actually show:
...
- **Evidence:** `governance/PRINCIPLES.md` design principles ranked, with the correct #1 (workflow before visuals) — a lesson the project *documented* before actually living it.
