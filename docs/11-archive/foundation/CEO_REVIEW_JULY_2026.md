...
| Design system (Phases 0–2) | `docs/11-archive/design/project-state-july-2026.md`, commit `9f7e7c4` | Genuinely improved; frozen credibly at 7.8/10 visual |

**What does not work (product level):**

1. **No single daily home.** App opens on Dashboard (`/`); execution lives on Workplace (`/workplace`). Documented in [ux-friction-review.md](../11-archive/design/ux-friction-review.md); confirmed in `page.tsx` and `sidebar-navigation.tsx`.
...
6. **Production build currently fails.** `npm run build` errors on `workplace-recover-day-bar.tsx` — imports non-existent `addMinutesToTime` from `date-utils.ts`. **Contradicts** [governance/GATES.md](./governance/GATES.md) checklist implying build passes.
...
| Launch readiness | 3/10 | **Lower than governance/GATES.md (4/10)** due to build failure + RLS |
...
The SRL cycle (forethought → performance → reflection) in [PRODUCT_VISION.md](./PRODUCT_VISION.md) and [../11-archive/project/02-related-works.md](../11-archive/project/02-related-works.md) is a coherent design framework. It is **not** a moat. Users do not search for "self-regulated learning apps." They search for less app switching and a clearer daily plan.
...
**No.** SRL is an **internal design lens**, not a user-facing differentiator. Keep it in [governance/PRINCIPLES.md](./governance/PRINCIPLES.md). Remove it from elevator pitches until the experience proves it.
...
The phrase is excellent **engineering direction** ([governance/PRINCIPLES.md](./governance/PRINCIPLES.md)). It is not yet **marketing copy** because the product contradicts it: two homes, three schedulers, modal capture, list-page detours.
...
| Friction | Impact |
|----------|--------|
| Planning model: group vs Today vs Later vs date | High — 10–30 decisions/day ([ux-friction-review.md](../11-archive/design/ux-friction-review.md) #5) |
...
The UX friction review ([ux-friction-review.md](../11-archive/design/ux-friction-review.md)) correctly identifies the problem. But the 6-sub-phase Phase 3 plan ([ROADMAP.md](../11-archive/design/roadmap-pre-masterplan.md)) is **too large before first external validation** — classic solo-founder planning comfort.
...
|----------|-------|------------|
| [PROJECT_STATE.md](../11-archive/design/project-state-july-2026.md) module map | `/` = Today/Workplace, `/overview` = Dashboard | `/` = Dashboard, `/workplace` = Workplace, no `/overview` |
| [governance/GATES.md](./governance/GATES.md) | Implies build passes | **`npm run build` fails** (TypeScript error) |
| [FEATURE_INVENTORY.md](./FEATURE_INVENTORY.md), friction review | Agenda card daily trust issue | Component **not mounted** — dead code |
| [governance/GATES.md](./governance/GATES.md) | RLS "verify" | Core tables **provably open** — must rewrite policies |
...
- [launch-plan-july-2026.md](../11-archive/planning/launch-plan-july-2026.md) — timeline to adjust after this review  
- [governance/GATES.md](./governance/GATES.md) — ratings revised downward here for build + RLS  
- [../11-archive/design/ux-friction-review.md](../11-archive/design/ux-friction-review.md) — workflow diagnosis **still valid**  
- [../11-archive/design/roadmap-pre-masterplan.md](../11-archive/design/roadmap-pre-masterplan.md) — Phase 3 direction valid; sequencing should change  
- [decision-log.md](../current-phase/logs/decision-log.md) — append outcomes from 90-day plan  
