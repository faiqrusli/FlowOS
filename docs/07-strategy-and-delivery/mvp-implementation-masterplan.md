...
- reconcile the Feature Catalog with code and the detailed `foundation/FEATURE_INVENTORY.md` reference;
...
|---|---|---|
| `docs/strategy/execution-masterplan.md` | **Superseded as implementation source; transitional historical context** | Extract only still-relevant work after it passes the Feature Catalog, current Vision, and a phase gate. |
| `docs/foundation/FEATURE_INVENTORY.md` | **Current implementation reference; not roadmap authority** | Reconcile its shipped/partial/deferred observations in Phase 1. |
| `docs/foundation/governance/GATES.md` | **Current reference requiring reconciliation** | Preserve valid security/quality gates; update any product-specific gate that conflicts with this plan through a decision record. |
| `docs/execution/runbooks/` | **Operational references** | Keep runbooks for current operations; create new runbooks only from an admitted delivery/release plan. |
