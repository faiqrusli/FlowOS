...
This sequence is intentionally outcome-level. The detailed pre-dogfood implementation sequence currently lives in the [MVP Implementation Masterplan](./mvp-implementation-masterplan.md). The older [execution-masterplan.md](../11-archive/strategy/execution-masterplan.md) is archived historical context only. The MVP masterplan must not add or reorder an outcome without an explicit Roadmap update and decision record.
...
The [MVP Implementation Masterplan](./mvp-implementation-masterplan.md) owns the current pre-dogfood implementation sequence, scope controls, and readiness gates. The older [execution-masterplan.md](../11-archive/strategy/execution-masterplan.md) is archived historical context only. This Roadmap owns the outcome sequence and gate logic to which the MVP masterplan and future delivery plans must link.
