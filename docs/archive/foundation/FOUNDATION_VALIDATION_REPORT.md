...
Verdict per dimension, with drift findings. Credit where due: the `archive/foundation/GOVERNANCE_PASS_REPORT.md` drift-fixing pass was real — several older claims (module map, agenda card, build status) were already corrected. But this board found additional drift the internal audits missed.
...
`archive/foundation/CEO_REVIEW_JULY_2026.md` states core tables are "provably open" (`using (true)`). What the SQL files actually show:
