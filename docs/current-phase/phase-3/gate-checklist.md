...
| G3-02 | Canonical Today workspace and route recovery work | Route regression evidence plus [redacted production entry probe](./evidence/today-orientation/production-entry-route-recovery.md) and pending seeded/real `/` workspace walkthrough, direct/re-entry/handoff/retry/recovery, accessibility, and owner-boundary evidence | `IN_PROGRESS` — anonymous route probe `PASS`; authenticated evidence `INCONCLUSIVE` |
| G3-03 | Tasks support commitment and action | [Tasks implementation evidence](./tasks-implementation-evidence.md), [redacted production task-drag record](./evidence/tasks-core-loop/production-task-drag.md), plus validation plan covering all `TASK-*` questions, lifecycle, selection unchanged-state, correction/defer/withdrawal, and recovery | `IN_PROGRESS` — local Later drag fix verified; authenticated production retest and remaining manual/live scenarios pending |
