...
Read: docs/12-team/active-6-role/[role-name].md
...
Implementation Engineer ─ Working code + Tests
  ↓ (hands off)
Release Manager ────── QA validation + Deployment
  ↓ (requests approval)
Founder ───────────── Approves + Authorizes release
```

**Total flow: 6 handoffs**
...
Input: Feature brief, Behavior contract, Design system
...
Input: Behavior contract, Design spec, Engineering architecture
...
Input: Complete feature dossier (brief, contract, design, delivery plan)
...
Input: Implementation PR, Validation plan, Behavior contract
