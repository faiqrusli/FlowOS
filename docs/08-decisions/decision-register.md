...
The register lists only decisions governed by [Decision Records](./decision-records.md). The historical [legacy decision log](../execution/logs/decision-log.md) remains separate and append-only; its entries are not silently converted into individual records.
