...
describe("timeline-task-focus", () => {
  it("is active only for the focused task", () => {
  ...
  it("keeps focus chrome during break (softened)", () => {
    const session = makeSession({ mode: "break" });
    expect(isTimelineTaskInFocus(session, "task-1")).toBe(true);
    expect(isTimelineTaskFocusSoftened(session, "task-1")).toBe(true);
  });

  it("keeps focus chrome while paused (softened)", () => {
    const session = makeSession({ session_status: "paused" });
    expect(isTimelineTaskInFocus(session, "task-1")).toBe(true);
    expect(isTimelineTaskFocusSoftened(session, "task-1")).toBe(true);
  });

  it("does not soften while actively focusing", () => {
    const session = makeSession();
    expect(isTimelineTaskFocusSoftened(session, "task-1")).toBe(false);
  });
